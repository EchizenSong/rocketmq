...
public class DefaultMQProducerImpl implements MQProducerInner {
    ...
    public void start(final boolean startFactory) throws MQClientException {
        switch (this.serviceState) {
            case CREATE_JUST:
                ...
                // 校验配置producer group合法性(主要判断是否为空、是否有非法字符、是否与默认group相同)
                this.checkConfig();
                ...
                /*
                 实例化客户端，一个JVM中只存在一个MQClientInstance，生产者与消费者公用，缓存在ConcurrentMap<String, MQClientInstance> factoryTable中
                 MQClientInstance封装了网络通信
                 */
                this.mQClientFactory = MQClientManager.getInstance().getOrCreateMQClientInstance(this.defaultMQProducer, rpcHook);
                // 将当前生产者实例注册到MQClientInstance，保存在内存中，方便后续进行网络请求、心跳检测
    ...
    private SendResult sendDefaultImpl(
        ...
        // 根据topic获取路由信息（如果为空，则从namesrv获取）
            ...
            for (; times < timesTotal; times++) {
                ...
                if (mqSelected != null) {
                    ...
                    try {
                        ...
                        // 在启用故障延迟机制时，用于修改broker是否需要被隔离，感觉没什么用，明明try-catch已经处理了
                        this.updateFaultItem(mq.getBrokerName(), endTimestamp - beginTimestampPrev, false);
                        // ONEWAY不管发送结果、异步这里拿不到结果，所以只要处理SYNC，默认同步发送
    ...
    private TopicPublishInfo tryToFindTopicPublishInfo(final String topic) {
        ...
        if (null == topicPublishInfo || !topicPublishInfo.ok()) {
            // 当topic路由信息为空，则创建一个空的TopicPublishInfo；然后从nameserver上获取最新的路由信息
        ...
        if (topicPublishInfo.isHaveTopicRouterInfo() || topicPublishInfo.ok()) {
            ...
            // 如果还是没有，则尝试用默认topic去查找；前提是设置broker属性autoCreateTopicEnable为true
    ...
    private SendResult sendKernelImpl(final Message msg,
        ...
        // producer初始化时，保存的brokerId - address的缓存
